...
class RAFT(nn.Module):
    # SEA-RAFT Medium defaults (from official princeton-vl/SEA-RAFT repo)
    _DEFAULTS = dict(
        dim=128, radius=4, iters=12,
        block_dims=[64, 128, 256], initial_dim=64,
        pretrain="resnet34", num_blocks=2,
        use_var=True, var_max=10, var_min=0,
    )

    def __init__(self, args=None, **kwargs):
        super().__init__()
        if args is None:
            # Build args from kwargs, filling in defaults
            from types import SimpleNamespace
            merged = {**self._DEFAULTS, **kwargs}
            # block_dims gets mutated in-place by ResNetFPN, so copy it
            merged["block_dims"] = list(merged["block_dims"])
