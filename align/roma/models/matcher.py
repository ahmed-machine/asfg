...
from align.roma.utils import get_tuple_transform_ops
from align.roma.utils.local_correlation import local_correlation
from align.roma.utils.utils import (
    check_rgb,
    cls_to_flow_refine,
    get_autocast_params,
    check_not_i16,
)
from align.roma.utils.kde import kde
from align.roma.models.encoders import CNNandDinov2
...
class GP(nn.Module):
    ...
    def forward(self, x, y, **kwargs):
        ...
        if self.training:
            # MPS Fix: inverse is often unstable or missing on MPS
            device = x.device
            if device.type == 'mps':
                K_yy_inv = torch.linalg.inv((K_yy + sigma_noise).cpu()).to(device)
            else:
                K_yy_inv = torch.linalg.inv(K_yy + sigma_noise)
            mu_x = K_xy.matmul(K_yy_inv.matmul(f))
        else:
            # faster inference, possibly also useful for training
            # MPS Fix: Cholesky solve is not implemented for MPS, fallback to CPU
            device = x.device
            if device.type == 'mps':
                K_yy_noise_cpu = (K_yy + sigma_noise).cpu()
                f_reshaped_cpu = f.reshape(b, h2 * w2, d).cpu()
                L_t_cpu = torch.linalg.cholesky(K_yy_noise_cpu)
                pos_emb = torch.cholesky_solve(f_reshaped_cpu, L_t_cpu, upper=False).to(device)
            else:
...
def _check_input(im_input):
    if isinstance(im_input, (str, os.PathLike)):
        ...
        if H % 14 != 0 or W % 14 != 0:
            H_new = (H // 14) * 14
            W_new = (W // 14) * 14
            im_input = F.interpolate(im_input, size=(H_new, W_new), mode='bilinear', align_corners=False)
        im = im_input
    return im
...
class RegressionMatcher(nn.Module):
    ...
    def sample(
        ...
        if certainty.sum() <= 0:
            return torch.zeros((0, 4), device=matches.device), torch.zeros((0,), device=certainty.device)
        
        expansion_factor = 4 if "balanced" in self.sample_mode else 1
        
        # Use a deterministic generator for reproducibility
        g = torch.Generator(device=certainty.device)
        g.manual_seed(42)
        ...
        balanced_samples = torch.multinomial(
            p, num_samples=min(num, len(good_certainty)), replacement=False, generator=g
        )
        ...
        *args,
        ...
        if self.upsample_preds and im_A_high_res is None and im_B_high_res is None:
            ...
            if isinstance(im_A_input, (str, os.PathLike)):
                ...
                # If already tensors, use as is (already normalized/device)
                im_A, im_B = im_A_input, im_B_input
                # Ensure they are at the correct upsample resolution if possible
                if im_A.shape[-2:] != (hs, ws):
                     im_A = F.interpolate(im_A, size=(hs, ws), mode='bilinear', align_corners=False)
                     im_B = F.interpolate(im_B, size=(hs, ws), mode='bilinear', align_corners=False)
            else:
            ...
            # Make sure we have the batch dimension if coming from single PIL/path
            if len(im_A.shape) == 3:
    ...
    def visualize_warp(
        ...
        if save_path is not None:
            from align.roma.utils import tensor_to_pil
