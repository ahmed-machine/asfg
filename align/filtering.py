...
from scipy.spatial import cKDTree, ConvexHull, Delaunay  # type: ignore
...
# ---------------------------------------------------------------------------
# TIN-TARR topological filtering (merged from tin_filter.py)
# ---------------------------------------------------------------------------
...
def filter_by_tin_tarr(matched_pairs, boundary_pairs, threshold=1.5):
    ...
    return [matched_pairs[i] for i in range(n_internal) if active[i]]


def _local_piecewise_warp_cv2(ref_pts_local, off_pts_local, off_img, output_shape):
    """Warp *off_img* using piecewise affine with OpenCV remap."""
    h, w = output_shape
    ...
    warped = cv2.remap(off_img, map_x, map_y, interpolation=cv2.INTER_LINEAR,
...
def _single_point_ad(pt_idx, ref_pts_px, off_pts_px, tin, hull_vertices_set,
                     ref_img, off_img):
    """Compute Accuracy Difference for one GCP."""
...
def optimize_fpps_accuracy(matched_pairs, boundary_pairs, arr_ref, arr_off,
    ...
    protected = set(i for i, p in enumerate(matched_pairs) if p.is_anchor)
    shore_removed = 0
    inland_removed = 0
