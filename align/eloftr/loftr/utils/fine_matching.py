...
class FineMatching(nn.Module):
    ...
    def forward(self, feat_0, feat_1, data):
        ...
        device_type = feat_0.device.type
        if device_type not in ['cuda', 'cpu', 'mps']:
            device_type = 'cpu'
        
        # Newer torch versions support mps autocast, but older might not.
        # Fallback to cpu if not cuda or mps supported.
        autocast_enabled = (not (self.training or self.validate))
        if device_type == 'mps' and not hasattr(torch, 'mps'):
             device_type = 'cpu'

        with torch.autocast(enabled=autocast_enabled, device_type=device_type):
