...
from .types import GCP, GlobalHypothesis, MatchPair, MetadataPrior, QaReport, gcps_from_legacy, match_pairs_from_legacy
...
def _make_sub_args(input_path, reference, output, match_res=5.0,
    ...
    return argparse.Namespace(
        ...
        match_res=match_res, coarse_pass=coarse_pass, best=best,
        anchors=anchors, yes=yes, device=device,
        tin_tarr_thresh=tin_tarr_thresh, skip_fpp=skip_fpp,
        matcher_anchor=matcher_anchor, matcher_dense=matcher_dense,
        grid_size=grid_size, grid_iters=grid_iters, arap_weight=arap_weight,
...
class AlignState:
    ...
    boundary_gcps: list = field(default_factory=list)
    geo_residuals: list = field(default_factory=list)
    ...
    qa_holdout_pairs: list = field(default_factory=list)
    match_records: List[MatchPair] = field(default_factory=list)
    gcp_records: List[GCP] = field(default_factory=list)
...
def step_coarse_offset(state: AlignState) -> AlignState:
    ...
    dx_c, dy_c, corr_c = detect_offset_at_resolution(
        ...
        template_radius_m=6000, mask_mode=state.mask_provider)
    if dx_c is None or dy_c is None or corr_c is None:
        print("ERROR: Could not detect offset at 15m resolution")
        src_offset.close()
        src_ref.close()
        sys.exit(1)
    ...
    dx_r, dy_r, corr_r = detect_offset_at_resolution(
        ...
        template_radius_m=6000, coarse_offset=(dx_c_f, dy_c_f), search_margin_m=300,
        mask_mode=state.mask_provider)
    ...
    # Pass 3: Independent grayscale NCC cross-check at 5m/px.
    ...
    try:
        from .image import clahe_normalize as _clahe
        arr_ref_xc, _ = read_overlap_region(
            src_ref, state.overlap, state.work_crs, 5.0)
        ...
        if np.any(valid_ref):
            ...
            if (templ_xc.shape[0] >= 64 and templ_xc.shape[1] >= 64 and
                    ...
                    search_xc.shape[1] > templ_xc.shape[1]):
                res_xc = cv2.matchTemplate(search_xc, templ_xc, cv2.TM_CCOEFF_NORMED)
                _, max_val_xc, _, max_loc_xc = cv2.minMaxLoc(res_xc)
                dx_xc = (s_c0_xc + max_loc_xc[0] - tc0_xc) * 5.0
                dy_xc = (s_r0_xc + max_loc_xc[1] - tr0_xc) * 5.0
                delta_from_mask = np.sqrt(
                    (dx_xc - state.coarse_dx) ** 2 + (dy_xc - state.coarse_dy) ** 2)
                print(f"    grayscale NCC: dx={dx_xc:+.0f}m, dy={dy_xc:+.0f}m "
                      f"(corr={max_val_xc:.4f}, delta={delta_from_mask:.0f}m from mask)")
                if max_val_xc > 0.3 and delta_from_mask < 150:
                    state.coarse_dx = float(dx_xc)
                    state.coarse_dy = float(dy_xc)
...
def step_handle_large_offset(state: AlignState, args) -> AlignState:
    ...
    if state.coarse_total < 10:
        if abs(state.expected_scale - 1.0) < 0.05:
            print("Images appear already well-aligned (offset < 10m, scale ~1.0).")
            sys.exit(0)
        else:
    ...
    if state.coarse_total > 2000:
        ...
        if state.coarse_pass >= max_coarse_passes:
            ...
            os.close(tmp_fd)
            try:
                refine_args = _make_sub_args(
                    input_path=state.output_path,
                    reference=state.reference_path,
                    output=tmp_path_refine,
                    match_res=state.match_res,
                    coarse_pass=state.coarse_pass + 1,
                    best=state.best,
                    anchors=state.anchors_path,
                    ...
                    mask_provider=state.mask_provider,
...
def step_feature_matching(state: AlignState, args) -> AlignState:
    ...
    if len(auto_pairs) >= 6:
        ...
        geo_dst = np.array([(m[2], m[3]) for m in auto_pairs],
                           dtype=np.float32).reshape(-1, 1, 2)
        geo_ransac_thresh = max(5.0 * neural_res, 20.0)
    ...
    state.match_records = match_pairs_from_legacy(state.matched_pairs)
...
def _ncc_match_worker(args):
    ...
    if np.mean(template > 0) < 0.3 or is_cloudy_patch(template):
        return None
    ...
    if np.mean(search > 0) < 0.3 or is_cloudy_patch(search):
        return None
...
def step_validate_and_filter(state: AlignState) -> AlignState:
    ...
    print()
    if len(state.matched_pairs) < 4:
        print("ERROR: Not enough matched points (need at least 4)")
        if state.precorrection_tmp and os.path.exists(state.precorrection_tmp):
            os.remove(state.precorrection_tmp)
        sys.exit(1)
    ...
    if len(state.matched_pairs) > 4:
        med_e = np.median(offsets_e)
        med_n = np.median(offsets_n)
        ...
        keep = []
        for i in range(len(state.matched_pairs)):
            is_anchor = state.matched_pairs[i][5].startswith("anchor:")
            if is_anchor or (abs(offsets_e[i] - med_e) < mad_sigma * sigma_e and
                             abs(offsets_n[i] - med_n) < mad_sigma * sigma_n):
                keep.append(i)
        if len(keep) >= 4 and len(keep) < len(state.matched_pairs):
            n_rejected = len(state.matched_pairs) - len(keep)
            state.matched_pairs = [state.matched_pairs[i] for i in keep]
    ...
    for pair in state.matched_pairs:
        ...
        state.gcps.append((col, row, rgx, rgy))
    ...
    # When the reference has a geographic shift from ground truth, LoFTR matches
    ...
    # Build boundary pairs in geo-geo format for TIN-TARR.
    # boundary_gcps are (px, py, gx, gy) where gx,gy = reference geo coords.
    # We need offset geo coords for the boundary -- derive from the global affine
    # inverse (ref -> offset).
    boundary_pairs_geo = []
    if state.M_geo is not None and len(state.boundary_gcps) > 0:
        # M_geo maps offset_geo -> ref_geo.  Invert to get ref_geo -> offset_geo.
        ...
        try:
            ...
            for bg in state.boundary_gcps:
                ...
                boundary_pairs_geo.append((ref_gx, ref_gy, off_gx, off_gy))
        except np.linalg.LinAlgError:
            pass

    # --- TIN-TARR topological filter (Phase A) ---
    if len(state.matched_pairs) >= 4 and boundary_pairs_geo:
        from .tin_filter import filter_by_tin_tarr
        ...
        if n_rejected_tin > 0:
            ...
            # Rebuild GCPs from surviving matched_pairs
            state.gcps = []
            for pair in state.matched_pairs:
                rgx, rgy, ogx, ogy = pair[0], pair[1], pair[2], pair[3]
                col, row = _to_pixel(ogx, ogy)
                state.gcps.append((col, row, rgx, rgy))
            # Recompute residuals
            if len(state.matched_pairs) >= 3 and state.M_geo is not None:
                state.geo_residuals = []
                for p in state.matched_pairs:
                    ogx, ogy = p[2], p[3]
                    rgx, rgy = p[0], p[1]
                    pred_x = state.M_geo[0, 0] * ogx + state.M_geo[0, 1] * ogy + state.M_geo[0, 2]
                    pred_y = state.M_geo[1, 0] * ogx + state.M_geo[1, 1] * ogy + state.M_geo[1, 2]
                    state.geo_residuals.append(
                        np.sqrt((pred_x - rgx) ** 2 + (pred_y - rgy) ** 2))
        else:
    ...
    if not state.skip_fpp and len(state.matched_pairs) >= 6 and boundary_pairs_geo:
        from .tin_filter import optimize_fpps_accuracy
