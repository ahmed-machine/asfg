...
Usage:
    python3 scripts/compare.py [--format text|json] [--baseline N]
"""
...
def format_text(rows, baseline_version=None):
    ...
    for row in rows:
        ...
        print(f"{prefix}v{v:>3}  {gcps:>4}  {g['west']:>3}m  {g['center']:>3}m  {g['east']:>4}m  {g['north']:>+4}m  {g['patch_med']:>4}m  {siou_str}  {shiou_str}  {g['score']:>5.0f}  {cv_str}  {g['total']:>5.0f}  {acc:>3}")
    ...
    if sb:
        ...
        for key in ["west_contrib", "center_contrib", "east_contrib", "north_contrib",
                     "patch_contrib", "stable_iou_penalty", "shore_iou_penalty"]:
            val = sb.get(key, 0)
