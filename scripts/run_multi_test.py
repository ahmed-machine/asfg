...
Usage:
    python3 scripts/run_multi_test.py [--skip-download] [--skip-georef] [--entity ENTITY_ID ...]
"""
...
def run_georef(entities=None):
    ...
    cmd = [
        PYTHON, str(PROJECT_ROOT / "process.py"),
        "--csv", *CSVS,
...
def run_stitch_qa(entity_id: str):
    ...
    if len(raw_frames) == 1:
        try:
            ...
            from declass.stitch import detect_subframe_seams
    ...
    if georef_path.exists() and os.path.exists(REFERENCE):
        try:
            sys.path.insert(0, str(PROJECT_ROOT))
            from declass.orientation import verify_orientation_against_reference
