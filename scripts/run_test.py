...
Usage:
    python3 scripts/run_test.py [--version N] [--timeout 2400]
"""
...
def parse_log(log_text):
    ...
    # Grid QA
    m = re.search(r"Grid warp QA:\s*west=(\d+)m\s+center=(\d+)m\s+east=(\d+)m\s+north=([+\-]?\d+)m\s+patch=(\d+)m\s+stable_iou=([\d.]+)\s+score=(\d+)", log_text)
    if m:
        result["grid_qa"] = {
            "west": int(m.group(1)), "center": int(m.group(2)),
            "east": int(m.group(3)), "north": int(m.group(4)),
            "patch_med": int(m.group(5)), "stable_iou": float(m.group(6)),
            "score": int(m.group(7)),
        }

    # TPS QA
    m = re.search(r"TPS fallback QA:\s*west=(\d+)m\s+center=(\d+)m\s+east=(\d+)m\s+north=([+\-]?\d+)m\s+patch=(\d+)m\s+stable_iou=([\d.]+)\s+score=(\d+)", log_text)
    if m:
        result["tps_qa"] = {
            "west": int(m.group(1)), "center": int(m.group(2)),
            "east": int(m.group(3)), "north": int(m.group(4)),
...
def build_summary(version, run_dir, log_text, exit_code, wall_clock_s, qa_path):
    ...
    def report_to_summary(report):
        ...
        s = {
            ...
            "west": round(im.get("west", 0)),
            "center": round(im.get("center", 0)),
            "east": round(im.get("east", 0)),
            "north": round(im.get("north_shift", 0)),
    ...
    if grid_summary is None and parsed.get("grid_qa"):
        ...
        grid_summary = {
            ...
            "accepted": True, "west": gq["west"], "center": gq["center"],
            "east": gq["east"], "north": gq["north"],
    ...
    if tps_summary is None and parsed.get("tps_qa"):
        ...
        tps_summary = {
            ...
            "accepted": True, "west": tq["west"], "center": tq["center"],
            "east": tq["east"], "north": tq["north"],
