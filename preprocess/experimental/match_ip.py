...
# NOTE: ``generate_subframe_pair_matches`` and
# ``generate_subframe_strip_matches`` were removed in Phase 4.5 of the
# recovery plan. They implemented pixel-crop-based overlap matching for
# sub-frame pairs without requiring valid USGS corners, but were never
# called anywhere in the live pipeline — retained as "may be useful
# later" scaffolding since April. The Phase 4 joint BA path feeds ASP
# absolute GCPs (per-sub-frame RoMa-vs-reference matches) + the
# existing ``generate_strip_matches`` tie-points, so the sub-frame
# pixel-crop variant is no longer the right tool for any live path.
# Reintroduce from git history if an inter-sub-frame tie-point matcher
