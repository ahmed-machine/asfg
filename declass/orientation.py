...
def interpolate_frame_corners(corners, frame_idx, n_frames):
    """Interpolate approximate corners for a single frame within a strip.

    Assumes frames are arranged left-to-right (west-to-east) in pixel space.
    Returns corners for the k-th frame (0-indexed) out of n_frames total.
    """
    nw = np.array(corners["NW"])
    ne = np.array(corners["NE"])
    sw = np.array(corners["SW"])
    se = np.array(corners["SE"])

    fx0 = frame_idx / n_frames
    fx1 = (frame_idx + 1) / n_frames

    return {
        "NW": tuple(nw * (1 - fx0) + ne * fx0),
        "NE": tuple(nw * (1 - fx1) + ne * fx1),
        "SW": tuple(sw * (1 - fx0) + se * fx0),
        "SE": tuple(sw * (1 - fx1) + se * fx1),
    }


def apply_rotation(image_path: str, output_path: str, degrees: int):
    """Rotate an image by the specified degrees (90, 180, 270).

    - 180°: Flip both axes (reuse flip_frame_180 logic)
    - 90° CW: Transpose + horizontal flip
    - 270° (90° CCW): Transpose + vertical flip
    """
    ...
    if os.path.exists(output_path):
        print(f"  [skip] Rotated output already exists: {output_path}")
        return output_path
    ...
    if degrees == 180:
        out_ds = driver.Create(output_path, w, h, n_bands, dt,
        ...
        # 90° CW: transpose then flip horizontally
        out_ds = driver.Create(output_path, h, w, n_bands, dt,
                               ["COMPRESS=LZW", "PREDICTOR=2", "TILED=YES",
                                "BIGTIFF=IF_SAFER"])
        for b in range(1, n_bands + 1):
            arr = ds.GetRasterBand(b).ReadAsArray()
            rotated = np.rot90(arr, k=-1)  # CW rotation
            out_ds.GetRasterBand(b).WriteArray(rotated)

    elif degrees == 270:
        # 90° CCW: transpose then flip vertically
        out_ds = driver.Create(output_path, h, w, n_bands, dt,
                               ["COMPRESS=LZW", "PREDICTOR=2", "TILED=YES",
                                "BIGTIFF=IF_SAFER"])
        for b in range(1, n_bands + 1):
            arr = ds.GetRasterBand(b).ReadAsArray()
            rotated = np.rot90(arr, k=1)  # CCW rotation
            out_ds.GetRasterBand(b).WriteArray(rotated)
    ...
    print(f"  Rotated {degrees}°: {os.path.basename(output_path)}")
    return output_path
